...
An interactive RAG based application built using FastAPI and Streamlit to explore and analyze publications from the CFA Institute Research Foundation. The application extracts content from the publication PDF documents, converts them into vector embeddings which will be stored in Vector Database, answers user queries specific to a publication with the help of cosine similarity that is checked between user question prompt and vector embeddings, generates report which includes response, images, graphs that are linked to the user prompt, and trustworthy score of the response is generated by Cleanlabs
